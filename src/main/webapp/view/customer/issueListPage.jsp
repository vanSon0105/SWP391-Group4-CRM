...
<body class="home-page">
	...
	<main>
		...
		<c:choose>
				...
				<table>
					...
					<tbody>
						<c:forEach var="s" items="${list}">
							<c:set var="status"
								value="${empty s.supportStatus ? 'new' : s.supportStatus}" />
							<tr>
								<td>${s.issueCode}</td>
								<td>${s.title}</td>
								<td><fmt:formatDate value="${s.createdAt}"
										pattern="dd/MM/yyyy HH:mm" /></td>
								<td><span class="status-pill status-${status}"> <c:choose>
										...
										test="${status == 'manager_rejected' && not empty s.feedback}">
										<div class="status-reason">Lý do: ${s.feedback}</div>
									</c:if></td>
								<td><c:if test="${status == 'awaiting_customer'}">
										<a class="action-link" href="issue-fill?id=${s.id}">Bổ
											sung form</a>
									</c:if> <c:if
										test="${status == 'awaiting_customer' || status == 'manager_rejected'}">
										<form method="post" action="issue-fill"
											style="display: inline;">
											<input type="hidden" name="issueId" value="${s.id}">
											<input type="hidden" name="cancel" value="1">
											<button type="submit" class="link-button" formnovalidate>Hủy
												yêu cầu</button>
										</form>
									</c:if></td>
							</tr>
			...
			<c:otherwise>
				<div class="empty-state">Bạn chưa có yêu cầu nào. Nhấn "Gửi
					yêu cầu mới" để mô tả vấn đề bạn gặp phải.</div>
			</c:otherwise>
		</c:choose>
