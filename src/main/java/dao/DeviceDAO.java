		...
		try (Connection conn = getConnection(); PreparedStatement ps = conn.prepareStatement(sql)) {
			...
			try (ResultSet rs = ps.executeQuery()) {
				while (rs.next()) {
					...
					Device d = new Device(rs.getInt("id"), c, rs.getString("name"), rs.getDouble("price"),
		...
		try (Connection conn = getConnection(); PreparedStatement pre = conn.prepareStatement(sql)) {
			...
			try (ResultSet rs = pre.executeQuery()) {
				if (rs.next()) {
					...
					return new Device(rs.getInt("id"), c, rs.getString("name"), rs.getDouble("price"),
							rs.getString("unit"), rs.getString("image_url"), rs.getString("description"),
							rs.getTimestamp("created_at"), rs.getBoolean("is_featured"));
				}
			}
		...
		try (Connection conn = getConnection();
			...
			while (rs.next()) {
				...
				list.add(new Device(rs.getInt("id"), c, rs.getString("name"), rs.getDouble("price"),
						rs.getString("unit"), rs.getString("image_url"), rs.getString("desciption"),
						rs.getTimestamp("created_at"), rs.getBoolean("is_featured")));
			}
...
//	Device - Homepage
	...
	public int getTotalBestSellingDevices() {
		int count = 0;
		String sql = "SELECT COUNT(DISTINCT d.id) " + "FROM order_details od "
				+ "JOIN devices d ON od.device_id = d.id ";
	...
	public List<Device> findAllDevices() {
		...
		try (Connection connection = getConnection();
			...
			while (rs.next()) {
				...
				list.add(new Device(rs.getInt("id"), c, rs.getString("name"), rs.getDouble("price"),
						rs.getString("image_url"), rs.getInt("inventory"), rs.getString("status")));
			}
		} catch (SQLException e) {
		...
		try (Connection connection = getConnection(); PreparedStatement pre = connection.prepareStatement(sql)) {
			...
			try (ResultSet rs = pre.executeQuery()) {
				if (rs.next()) {
					...
					return new Device(rs.getInt("id"), c, rs.getString("name"), rs.getDouble("price"),
							rs.getString("image_url"), rs.getString("unit"), rs.getString("description"),
							rs.getTimestamp("created_at"), rs.getInt("stock_quantity"));
				}
			}
		} catch (SQLException e) {
			e.printStackTrace();
